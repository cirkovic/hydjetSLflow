...
outdir = '~/www/21-05-2016/hydjet/3a'
...
def loop(s, n):
    ...
    for col, c in enumerate(centralities):
        with open(str(s)+'/'+c+'/'+c+'/data_'+n+'/clean_eigenvalues_7x7.json') as df:
            data = json.load(df)
    ...
    ls[-1].Draw()
    cs[-1].SetLogy()
    cs[-1].Print(outdir+'/'+str(s)+'/'+n+'/evs_'+n+'.png')
    ...
    for mode in ['mode1', 'mode2']:
        ...
        for col, c in enumerate(centralities):
            mlines = None
            #elines = None
            with open(str(s)+'/'+c+'/'+c+'/data_'+n+'/modes/'+mode+'.dat') as fm:
                mlines = fm.read().splitlines()
            with open(str(s)+'/'+c+'/'+c+'/data_'+n+'/errors/'+mode+'_error.dat') as fe:
        ...
        ls[-1].Draw()
        cs[-1].Print(outdir+'/'+str(s)+'/'+n+'/'+mode+'_'+n+'.png')
    ...
    for col, c in enumerate(centralities):
        ...
        ls[-1].Draw()
        cs[-1].Print(outdir+'/'+str(s)+'/'+n+'/m1_vs_m2_'+n+'_'+c+'.png')

    for c in centralities:
        with open(str(s)+'/'+c+'/'+c+'/data_'+n+'/Matrix_'+n+'_'+c+'_7x7.json') as df:
            data = json.load(df)
            ...
            hs[-1].SetStats(False)
            hs[-1].Draw("COLZ TEXT")
            cs[-1].Print(outdir+'/'+str(s)+'/'+n+'/M_'+n+'_'+c+'.png')
...
if __name__ == "__main__":
    ...
    for s in switches:
        for n in ['n2', 'n3']:
            ps.append(Process(target=loop, args=(s, n)))
            ps[-1].start()
