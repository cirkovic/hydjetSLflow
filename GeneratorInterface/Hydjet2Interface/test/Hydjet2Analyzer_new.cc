// -*- C++ -*-
//
// Package: Hydjet2Analyzer_new
// Class: Hydjet2Analyzer_new
//
...
class Hydjet2Analyzer_new : public edm::EDAnalyzer {
public:
  explicit Hydjet2Analyzer_new(const edm::ParameterSet&);
  ~Hydjet2Analyzer_new();
private:
...
//
Hydjet2Analyzer_new::Hydjet2Analyzer_new(const edm::ParameterSet& iConfig)
{
...
Hydjet2Analyzer_new::~Hydjet2Analyzer_new()
{
...
TString Hydjet2Analyzer_new::ToString(float num){
...
int Hydjet2Analyzer_new::myrandom (int i) { return std::rand()%i;}

//
// ------------ method called to for each event ------------
void
Hydjet2Analyzer_new::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  /*
  if (MY_CODE == 2) {
      using namespace edm;
...
void
Hydjet2Analyzer_new::beginRun(const edm::Run&, const edm::EventSetup& iSetup)
{
  iSetup.getData(pdt);
}
void
Hydjet2Analyzer_new::beginJob()
{
  if(printLists_){
...
void Hydjet2Analyzer_new:: DO_BACKGROUND2(EVENT* EVENTS, vector<pair<int, int>> PAIRS, string Noff){
...
void Hydjet2Analyzer_new:: CORRELATION_OF_MIXED_EVENTS(map<string, vector<TRIPLET>> PT_FIXED, map<string, vector<TRIPLET>> PT_CURRENT, string Noff_name){
...
void
Hydjet2Analyzer_new::endJob() {
}
//define this as a plug-in
DEFINE_FWK_MODULE(Hydjet2Analyzer_new);
