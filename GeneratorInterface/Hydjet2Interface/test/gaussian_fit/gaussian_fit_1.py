...
#for i in xrange(3, 4):
    ...
    hint = hs[-1].Integral()
    fns.append(TF1("gfs"+str(i), gfs, -1.0, 1.0))
    #f.SetParameters(1.0, 0.1, 0.0, 0.2, 0.6, 0.2, -0.6)
    #fns[-1].SetParameters(1.0, 0.1, 0.0, 1.0, 0.2, 0.6, 1.0, 0.2, -0.6)
    pars.append([])
    for k in xrange(0, Ntry):
        pars[-1].append(([], []))
        ...
        cs.append(TCanvas())
        #hs[-1].Draw("HIST E0")
        hs[-1].Fit("gfs"+str(i), "M")
        #pars[-1][-1][1].append(fns[-1].GetNDF())
        del pars[-1][-1][0][:]
        for j in xrange(0, Ngs):
            pars_temp = [fns[-1].GetParameter(3*j+0), fns[-1].GetParameter(3*j+1), fns[-1].GetParameter(3*j+2)]
            pars[-1][-1][0].append(pars_temp)
        pars[-1][-1][1].append(fns[-1].Integral(-1.0, 1.0))
        pars[-1][-1][1].append(hint)
        pars[-1][-1][1].append(fns[-1].GetChisquare())
