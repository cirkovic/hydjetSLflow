// -*- C++ -*-
//
// Package: Hydjet2Analyzer_new_my_502_pt_mult
// Class: Hydjet2Analyzer_new_my_502_pt_mult
//
...
class Hydjet2Analyzer_new_my_502_pt_mult : public edm::EDAnalyzer {
public:
  explicit Hydjet2Analyzer_new_my_502_pt_mult(const edm::ParameterSet&);
  ~Hydjet2Analyzer_new_my_502_pt_mult();
private:
...
//
Hydjet2Analyzer_new_my_502_pt_mult::Hydjet2Analyzer_new_my_502_pt_mult(const edm::ParameterSet& iConfig)
{
  ...
  */
  ...
  fSqrtS = iConfig.getUntrackedParameter<double>("maxEvents", -99);
  int VAR = -99;

  cout << "//////////////////////////" << endl;
  cout << "//" << endl;
  cout << "  Number of events to generate" << endl << VAR << endl;
  cout << "  C.m.s. energy per nucleon pair, fSqrtS [GeV]" << endl << fSqrtS << endl;
  exit(0);
  cout << "  Atomic weigth of nuclei, fAw" << endl << VAR << endl;
  cout << "  Flag of type of centrality generation, fBfix (=0 is fixed by fBfix, >0 distributed [fBfmin, fBmax])" << endl << VAR << endl;
  cout << "  Minimum impact parameter in units of nuclear radius, fBmin" << endl << VAR << endl;
  cout << "  Maximum impact parameter in units of nuclear radius, fBmax" << endl << VAR << endl;
  cout << "  Fixed impact parameter in units of nuclear radius, fBfix" << endl << VAR << endl;
  cout << "  Parameter to set random number seed, fSeed (=0 the current time is used, >0 the value fSeed is used) " << endl << VAR << endl;
  cout << "  Temperature at chemical freeze-out, fT [GeV]" << endl << VAR << endl;
  cout << "  Chemical baryon potential per unit charge, fMuB [GeV]" << endl << VAR << endl;
  cout << "  Chemical strangeness potential per unit charge, fMuS [GeV]" << "  " << endl << VAR << endl;
  cout << "  Chemical charm potential per unit charge, fMuC [GeV] (used if charm production is turned on)" << endl << VAR << endl;
  cout << "  Chemical isospin potential per unit charge, fMuI3 [GeV]" << endl << VAR << endl;
  cout << "  Temperature at thermal freeze-out, fTthFO [GeV]" << endl << VAR << endl;
  cout << "  Chemical potential of pi+ at thermal freeze-out, fMu_th_pip [GeV] " << endl << VAR << endl;
  cout << "  Proper time proper at thermal freeze-out for central collisions, fTau [fm/c]" << endl << VAR << endl;
  cout << "  Duration of emission at thermal freeze-out for central collisions, fSigmaTau [fm/c]" << endl << VAR << endl;
  cout << "  Maximal transverse radius at thermal freeze-out for central collisions, fR [fm]" << endl << VAR << endl;
  cout << "  Maximal longitudinal flow rapidity at thermal freeze-out, fYlmax" << endl << VAR << endl;
  cout << "  Maximal transverse flow rapidity at thermal freeze-out for central collisions, fUmax" << endl << VAR << endl;
  cout << "  Momentum azimuthal anizotropy parameter at thermal freeze-out, fDelta" << endl << VAR << endl;
  cout << "  Spatial azimuthal anisotropy parameter at thermal freeze-out, fEpsilon" << endl << VAR << endl;
  cout << "  Flag to specify fDelta and fEpsilon values, fIfDeltaEpsilon (=0 user's ones, >=1 calculated) " << endl << VAR << endl;
  cout << "  Flag to switch on/off hadron decays, fDecay (=0 decays off, >=1 decays on)" << endl << VAR << endl;
  cout << "  Low decay width threshold fWeakDecay[GeV]: width<fWeakDecay decay off, width>=fDecayWidth decay on; can be used to switch off weak decays" << endl << VAR << endl;
  cout << "  Flag to choose rapidity distribution, fEtaType (=0 uniform, >0 Gaussian with the dispersion Ylmax)" << endl << VAR << endl;
  cout << "  Flag to use calculated T_ch, mu_B and mu_S as a function of fSqrtS, fTMuType (=0 user's ones, >0 calculated) " << endl << VAR << endl;
  cout << "  Strangeness supression factor gamma_s with fCorrS value (0<fCorrS <=1, if fCorrS <= 0 then it is calculated)" << endl << VAR << endl;
  cout << "  Flag to include thermal charm production, fIcharm (=0 no charm production, >=1 charm production) " << endl << VAR << endl;
  cout << "  Charmness enhancement factor gamma_c with fCorrC value (fCorrC >0, if fCorrC<0 then it is calculated)" << endl << VAR << endl;
  cout << "  Flag to include jet (J)/jet quenching (JQ) and hydro (H) state production, fNhsel (0 H on & J off, 1 H/J on & JQ off, 2 H/J/HQ on, 3 J on & H/JQ off, 4 H off & J/JQ on)" << endl << VAR << endl;
  cout << "  Flag to suppress the output of particle history from PYTHIA, fIedit (=1 only final state particles; =0 full particle history from PYTHIA)" << endl << VAR << endl;
  cout << "  Flag to switch on/off nuclear shadowing, fIshad (0 shadowing off, 1 shadowing on)" << endl << VAR << endl;
  cout << "  Minimal pt of parton-parton scattering in PYTHIA event, fPtmin [GeV/c] " << endl << VAR << endl;
  cout << "  Initial QGP temperature for central Pb+Pb collisions in mid-rapidity, fT0 [GeV] " << endl << VAR << endl;
  cout << "  Proper QGP formation time in fm/c, fTau0 (0.01<fTau0<10)" << endl << VAR << endl;
  cout << "  Number of active quark flavours in QGP, fNf (0, 1, 2 or 3)" << endl << VAR << endl;
  cout << "  Flag to fixe type of partonic energy loss, fIenglu (0 radiative and collisional loss, 1 radiative loss only, 2 collisional loss only)" << endl << VAR << endl;
  cout << "  Flag to fix type of angular distribution of in-medium emitted gluons, fIanglu (0 small-angular, 1 wide-angular, 2 collinear)" << endl << VAR << endl;
  cout << "//" << endl;
  cout << "//////////////////////////" << endl;

}
Hydjet2Analyzer_new_my_502_pt_mult::~Hydjet2Analyzer_new_my_502_pt_mult()
{
...
TString Hydjet2Analyzer_new_my_502_pt_mult::ToString(float num){
...
int Hydjet2Analyzer_new_my_502_pt_mult::myrandom (int i) { return std::rand()%i;}

//
// ------------ method called to for each event ------------
void
Hydjet2Analyzer_new_my_502_pt_mult::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  /*
  if (MY_CODE == 2) {
      using namespace edm;
...
void
Hydjet2Analyzer_new_my_502_pt_mult::beginRun(const edm::Run&, const edm::EventSetup& iSetup)
{
  iSetup.getData(pdt);
}
void
Hydjet2Analyzer_new_my_502_pt_mult::beginJob()
{
  if(printLists_){
  ...
  */
  ...
  hmult = new TH1F("hmult", "hmult", 100, 0.0, 8000);
...
void Hydjet2Analyzer_new_my_502_pt_mult:: DO_BACKGROUND2(EVENT* EVENTS, vector<pair<int, int>> PAIRS, string Noff){
...
void Hydjet2Analyzer_new_my_502_pt_mult:: CORRELATION_OF_MIXED_EVENTS(map<string, vector<TRIPLET>> PT_FIXED, map<string, vector<TRIPLET>> PT_CURRENT, string Noff_name){
...
void
Hydjet2Analyzer_new_my_502_pt_mult::endJob() {
}
//define this as a plug-in
DEFINE_FWK_MODULE(Hydjet2Analyzer_new_my_502_pt_mult);
