// -*- C++ -*-
//
// Package: Hydjet2Analyzer_new_my_502
// Class: Hydjet2Analyzer_new_my_502
//
...
class Hydjet2Analyzer_new_my_502 : public edm::EDAnalyzer {
public:
  explicit Hydjet2Analyzer_new_my_502(const edm::ParameterSet&);
  ~Hydjet2Analyzer_new_my_502();
private:
...
//
Hydjet2Analyzer_new_my_502::Hydjet2Analyzer_new_my_502(const edm::ParameterSet& iConfig)
{
  ...
  fout = 0;
   ...
   int Nbins=iConfig.getParameter<int>("Nbins");
  ...
  if (DO_NONDIAG==true){
        ...
        for (int i=1; i<Nbins+1; i++){   //MAKE NON_DIAG. PAIRS
  ...
  if (DO_DIAG==true){
      ...
      for (int i=1; i<Nbins+1; i++){    //DIAG. PAIRS
...
Hydjet2Analyzer_new_my_502::~Hydjet2Analyzer_new_my_502()
{
...
TString Hydjet2Analyzer_new_my_502::ToString(float num){
...
int Hydjet2Analyzer_new_my_502::myrandom (int i) { return std::rand()%i;}

//
// ------------ method called to for each event ------------
void
Hydjet2Analyzer_new_my_502::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  /*
  if (MY_CODE == 2) {
      using namespace edm;
   ...
   */
    ...
    */
     ...
     for (unsigned int bin_pair=0; bin_pair<BIN_PAIRS.size(); bin_pair++){//BIN COMBINATIONS****************************
              ...
              Hmult1L[bin_pair]->Fill(PT[first_bin].size());
              Hmult2L[bin_pair]->Fill(PT[second_bin].size());

              for (unsigned int i=0; i<PT[first_bin].size(); i++){

                    for (unsigned int j=0; j<PT[second_bin].size(); j++){

                                   if (j == 0) Hpt1L[bin_pair]->Fill(PT[first_bin][i].pt);
                                   if (i == 0) Hpt2L[bin_pair]->Fill(PT[second_bin][j].pt);
...
void
Hydjet2Analyzer_new_my_502::beginRun(const edm::Run&, const edm::EventSetup& iSetup)
{
  iSetup.getData(pdt);
}
void
Hydjet2Analyzer_new_my_502::beginJob()
{
  if(printLists_){
  ...
  */
  ...
  Hmult1L = new TH1F*[nHarr];
  Hmult2L = new TH1F*[nHarr];
  Hpt1L = new TH1F*[nHarr];
  Hpt2L = new TH1F*[nHarr];
...
void Hydjet2Analyzer_new_my_502:: DO_BACKGROUND2(EVENT* EVENTS, vector<pair<int, int>> PAIRS, string Noff){
...
void Hydjet2Analyzer_new_my_502:: CORRELATION_OF_MIXED_EVENTS(map<string, vector<TRIPLET>> PT_FIXED, map<string, vector<TRIPLET>> PT_CURRENT, string Noff_name){
...
void
Hydjet2Analyzer_new_my_502::endJob() {
}
//define this as a plug-in
DEFINE_FWK_MODULE(Hydjet2Analyzer_new_my_502);
