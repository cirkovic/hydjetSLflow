// -*- C++ -*-
//
// Package: Hydjet2Analyzer_new_pt
// Class: Hydjet2Analyzer_new_pt
//
...
class Hydjet2Analyzer_new_pt : public edm::EDAnalyzer {
public:
  explicit Hydjet2Analyzer_new_pt(const edm::ParameterSet&);
  ~Hydjet2Analyzer_new_pt();
private:
...
//
Hydjet2Analyzer_new_pt::Hydjet2Analyzer_new_pt(const edm::ParameterSet& iConfig)
{
...
Hydjet2Analyzer_new_pt::~Hydjet2Analyzer_new_pt()
{
...
TString Hydjet2Analyzer_new_pt::ToString(float num){
...
int Hydjet2Analyzer_new_pt::myrandom (int i) { return std::rand()%i;}

//
// ------------ method called to for each event ------------
void
Hydjet2Analyzer_new_pt::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  /*
  if (MY_CODE == 2) {
      using namespace edm;
...
void
Hydjet2Analyzer_new_pt::beginRun(const edm::Run&, const edm::EventSetup& iSetup)
{
  iSetup.getData(pdt);
}
void
Hydjet2Analyzer_new_pt::beginJob()
{
  if(printLists_){
...
void Hydjet2Analyzer_new_pt:: DO_BACKGROUND2(EVENT* EVENTS, vector<pair<int, int>> PAIRS, string Noff){
...
void Hydjet2Analyzer_new_pt:: CORRELATION_OF_MIXED_EVENTS(map<string, vector<TRIPLET>> PT_FIXED, map<string, vector<TRIPLET>> PT_CURRENT, string Noff_name){
...
void
Hydjet2Analyzer_new_pt::endJob() {
}
//define this as a plug-in
DEFINE_FWK_MODULE(Hydjet2Analyzer_new_pt);
