// -*- C++ -*-
//
// Package: Hydjet2Analyzer_count
// Class: Hydjet2Analyzer_count
//
...
class Hydjet2Analyzer_count : public edm::EDAnalyzer {
public:
  explicit Hydjet2Analyzer_count(const edm::ParameterSet&);
  ~Hydjet2Analyzer_count();
private:
...
//
Hydjet2Analyzer_count::Hydjet2Analyzer_count(const edm::ParameterSet& iConfig)
{
...
Hydjet2Analyzer_count::~Hydjet2Analyzer_count()
{
...
TString Hydjet2Analyzer_count::ToString(float num){
...
int Hydjet2Analyzer_count::myrandom (int i) { return std::rand()%i;}

//
// ------------ method called to for each event ------------
void
Hydjet2Analyzer_count::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  if (MY_CODE == 3) {
          ...
          for(HepMC::GenEvent::particle_const_iterator it = evt->particles_begin(); it != evt->particles_end(); ++it){
            if((*it)->status() == 1){
              ...
              if (part) {
                  ...
                  std::cout << "PDG ID: " << pdg_id << ", charge: " << part->charge() << std::endl;
                  continue;
...
void
Hydjet2Analyzer_count::beginRun(const edm::Run&, const edm::EventSetup& iSetup)
{
  iSetup.getData(pdt);
}
void
Hydjet2Analyzer_count::beginJob()
{
  if(printLists_){
...
void Hydjet2Analyzer_count:: DO_BACKGROUND2(EVENT* EVENTS, vector<pair<int, int>> PAIRS, string Noff){
...
void Hydjet2Analyzer_count:: CORRELATION_OF_MIXED_EVENTS(map<string, vector<TRIPLET>> PT_FIXED, map<string, vector<TRIPLET>> PT_CURRENT, string Noff_name){
...
void
Hydjet2Analyzer_count::endJob() {
}
//define this as a plug-in
DEFINE_FWK_MODULE(Hydjet2Analyzer_count);
