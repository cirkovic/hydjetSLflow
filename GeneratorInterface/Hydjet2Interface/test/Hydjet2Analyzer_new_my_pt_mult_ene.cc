// -*- C++ -*-
//
// Package: Hydjet2Analyzer_new_my_pt_mult_ene
// Class: Hydjet2Analyzer_new_my_pt_mult_ene
//
...
class Hydjet2Analyzer_new_my_pt_mult_ene : public edm::EDAnalyzer {
public:
  explicit Hydjet2Analyzer_new_my_pt_mult_ene(const edm::ParameterSet&);
  ~Hydjet2Analyzer_new_my_pt_mult_ene();
private:
...
//
Hydjet2Analyzer_new_my_pt_mult_ene::Hydjet2Analyzer_new_my_pt_mult_ene(const edm::ParameterSet& iConfig)
{
...
Hydjet2Analyzer_new_my_pt_mult_ene::~Hydjet2Analyzer_new_my_pt_mult_ene()
{
...
TString Hydjet2Analyzer_new_my_pt_mult_ene::ToString(float num){
...
int Hydjet2Analyzer_new_my_pt_mult_ene::myrandom (int i) { return std::rand()%i;}

//
// ------------ method called to for each event ------------
void
Hydjet2Analyzer_new_my_pt_mult_ene::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  /*
  if (MY_CODE == 2) {
      using namespace edm;
          ...
          for(HepMC::GenEvent::particle_const_iterator it = evt->particles_begin(); it != evt->particles_end(); ++it){
            if((*it)->status() == 1){
              ...
              if (part) {
                  ...
                  if (abs(pdg_id) == 211 && pt > 0.3 && pt < 3.0 && abs(eta) < 2.4) {
...
void
Hydjet2Analyzer_new_my_pt_mult_ene::beginRun(const edm::Run&, const edm::EventSetup& iSetup)
{
  iSetup.getData(pdt);
}
void
Hydjet2Analyzer_new_my_pt_mult_ene::beginJob()
{
  if(printLists_){
...
void Hydjet2Analyzer_new_my_pt_mult_ene:: DO_BACKGROUND2(EVENT* EVENTS, vector<pair<int, int>> PAIRS, string Noff){
...
void Hydjet2Analyzer_new_my_pt_mult_ene:: CORRELATION_OF_MIXED_EVENTS(map<string, vector<TRIPLET>> PT_FIXED, map<string, vector<TRIPLET>> PT_CURRENT, string Noff_name){
...
void
Hydjet2Analyzer_new_my_pt_mult_ene::endJob() {
}
//define this as a plug-in
DEFINE_FWK_MODULE(Hydjet2Analyzer_new_my_pt_mult_ene);
