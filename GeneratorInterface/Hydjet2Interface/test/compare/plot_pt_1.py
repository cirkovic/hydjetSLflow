...
energies = [('../pt_276/', '276'), ('../pt_502/', '502')]
single_hpt = True
...
for ei, e in enumerate(energies):
    for ci, c in enumerate(centralities):
        #print c
        fs.append(TFile.Open(e[0]+'output_100_2_'+c+'_2.0.root'))
        #fs[-1].ls()
        #pta = []
        if single_hpt:
            hn = 'hpt'
        ...
        cs.append(TCanvas())
        Hs[-1].Draw()
        cs[-1].Print('~/www/13-07-2016/hydjet/compare_pt_1/'+Hs[-1].GetName()+'_'+c+'_'+e[1]+".png")
        #print
...
for ci, c in enumerate(centralities):
    ...
    Hs1[1][ci].Draw("SAME")
    cs[-1].Print('~/www/13-07-2016/hydjet/compare_pt_1/'+Hs[-1].GetName()+'_'+c+'_compare.png')
