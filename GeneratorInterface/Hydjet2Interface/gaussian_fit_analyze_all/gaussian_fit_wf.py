...
#WWW='/afs/cern.ch/user/c/cirkovic/www/12-08-2016_temp/'
#WWW='/afs/cern.ch/user/c/cirkovic/www/15-09-2016_temp/'
WWW='/afs/cern.ch/user/c/cirkovic/www/16-09-2016_temp/'
    ...
    for centrality in ['00_02', '00_05', '00_10', '10_20', '20_30', '30_40', '40_50', '50_60']:
        for n in ['n2', 'n3']:
            for mode in ['mode_2']:
                ...
                label = "276" if energy == "2.76TeV" else "502" if energy == "5.02TeV" else ""
                fs.append(TFile.Open("../macros_PROBE_sumCosL_multL_modified_mix_del_fix_2_6e6_proc_jovan_all_JOVAN_data_fit_40M_16-09-2016_"+label+"/2/2.0/"+centrality+"/"+centrality+"/data_"+n+"/pca_data.root"))
                fs0.append(TFile.Open("../macros_PROBE_sumCosL_multL_modified_mix_del_fix_2_6e6_proc_jovan_all_JOVAN_data_fit_40M_16-09-2016_"+label+"_analyze_0/2/2.0/"+centrality+"/"+centrality+"/data_"+n+"/pca_data.root"))
                '''
                fs[-1].ls()
                for i in xrange(0, 7):
...
#                    hs[-1] = hs[-1].Rebin(200)
                ...
                for i in xrange(0, 7):
                    ...
                    hs0.append(fs0[-1].Get(mode+"/ei_histo_1D"+str(i)))
                    hs[-1] = hs[-1].Rebin(200)
                    hs0[-1] = hs0[-1].Rebin(200)
                    hint = 0
...
#                        sigma = random.uniform(0.0, 1.0-abs(mean))
                    ...
                    hs0_temp = hs0[-1].Clone()
                    hs0_temp.Scale(hs[-1].GetMaximum()/hs0_temp.GetMaximum())
                    hs0_temp.SetFillColor(kMagenta)
                    hs0_temp.Draw("SAME")
