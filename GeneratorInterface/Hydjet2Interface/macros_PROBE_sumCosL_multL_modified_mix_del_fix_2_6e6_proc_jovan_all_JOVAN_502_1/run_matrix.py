...
def GET_MATRIX(error_matrix, BINS):
  ...
  for bin in BINS:
       ...
       bin_value=error_matrix["dV_"+str(bin[0])+"_"+str(bin[1])]

       random_error_value=random.uniform(-bin_value, bin_value)

       generated_matrix["dV_"+str(bin[0])+"_"+str(bin[1])]=random_error_value
...
def SUM_MATRIX(matrix1, matrix2, BINS):
      ...
      for bin in BINS:
            ...
            temp_sum=matrix1["dV_"+str(bin[0])+"_"+str(bin[1])]+matrix2["V_"+str(bin[0])+"_"+str(bin[1])]
       
            sum_matrix["V_"+str(bin[0])+"_"+str(bin[1])]=temp_sum
