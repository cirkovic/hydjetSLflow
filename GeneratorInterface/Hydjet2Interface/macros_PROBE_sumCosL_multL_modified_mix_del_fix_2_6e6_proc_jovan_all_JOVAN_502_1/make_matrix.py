...
def GET_MATRIX(SIGNAL, BACKGROUND = None):
   ...
   for bin in BINS:
          ...
          fracS=FRACTION["V_"+str(bin[0])+"_"+str(bin[1])]
          #fracS = 1
          #fracB=FRACTION_BACK["V_"+str(bin[0])+"_"+str(bin[1])]


          #Vpipj= 5*(fracS)*(SIGNAL["V_"+str(bin[0])+"_"+str(bin[1])]) - 50*(fracB)*(BACKGROUND["V_"+str(bin[0])+"_"+str(bin[1])])
          Vpipj= 5*(fracS)*(SIGNAL["V_"+str(bin[0])+"_"+str(bin[1])])# - 50*(fracB)*(BACKGROUND["V_"+str(bin[0])+"_"+str(bin[1])])
          Vpipj= 5*(fracS)*(SIGNAL["V_"+str(bin[0])+"_"+str(bin[1])])# - 50*(fracB)*(BACKGROUND["V_"+str(bin[0])+"_"+str(bin[1])])
                     
          MATRIX["V_"+str(bin[0])+"_"+str(bin[1])]=Vpipj

          if bin[0]!=bin[1]:

                   MATRIX["V_"+str(bin[1])+"_"+str(bin[0])]=Vpipj
...
def GET_MATRIX_ERROR(sig_error, back_error = None):
        ...
        for bin in BINS: 
               ...
               error_sig=math.pow(FRACTION["V_"+str(bin[0])+"_"+str(bin[1])], 2)*math.pow(sig_error["V_"+str(bin[0])+"_"+str(bin[1])],2)
               #error_back=math.pow(FRACTION_BACK["V_"+str(bin[0])+"_"+str(bin[1])], 2)*math.pow(back_error["V_"+str(bin[0])+"_"+str(bin[1])],2) 

             
               #matrix["dV_"+str(bin[0])+"_"+str(bin[1])]=5*math.sqrt(error_sig + 100*error_back)
               matrix["dV_"+str(bin[0])+"_"+str(bin[1])]=5*math.sqrt(error_sig)

               #if bin[0]!=bin[1]: matrix["dV_"+str(bin[1])+"_"+str(bin[0])]=5*math.sqrt(error_sig + 100*error_back)
               if bin[0]!=bin[1]: matrix["dV_"+str(bin[1])+"_"+str(bin[0])]=5*math.sqrt(error_sig)
...
def GET_COS_VALUE(path, f, Q):
      ...
      for bin in BINS:
        ...
        Q["V_"+str(bin[0])+"_"+str(bin[1])]=temp
...
def main():
   ...
   for bin in BINS:
         ...
         EVENT_NUMBER["P"+str(bin[0])+"_"+str(bin[1])]=temp

   for bin in BINS:

         temp=histo_back.GetBinContent(bin[0], bin[1])
         EVENT_NUMBER_BACK["P"+str(bin[0])+"_"+str(bin[1])]=temp
   ...
   for bin in BINS:
         ...
         temp_average=temp/EVENT_NUMBER["P"+str(bin[0])+"_"+str(bin[1])]

         FRACTION["V_"+str(bin[0])+"_"+str(bin[1])]=1.0/temp_average

   for bin in BINS:


         temp=histo_fraction_back.GetBinContent(bin[0], bin[1])
         temp_average=temp/EVENT_NUMBER_BACK["P"+str(bin[0])+"_"+str(bin[1])]

         FRACTION_BACK["V_"+str(bin[0])+"_"+str(bin[1])]=1.0/temp_average
   ...
   SUB_folder_n2=centrality+"/data_n2"
   SUB_folder_n3=centrality+"/data_n3"
   ...
   File2_error=open("./"+ centrality+"/data_n2/Error_n2_"+centrality+ "_"+RANGE+".json", "w")
   File3_error=open("./"+ centrality+"/data_n3/Error_n3_"+centrality+ "_"+RANGE +".json", "w")
